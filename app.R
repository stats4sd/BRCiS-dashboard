...
prepareData <- function(midline, backcheck){
  ...
  for(i in 1:length(var_to_backcheck)){
    ...
    if(var_to_backcheck[i] %in% c("HouseholdSize")){
    ...
    if(length(d_midline)>1 & length(d_midline)>1 & length(d_community)>1){
      ...
      midline <- as.data.frame(d_midline) %>%
        dplyr::rename(
          ...
          head_age=HoH_meta.update.b_106,
          head_marital_status=HoH_meta.update.b_107,
          worked_with_community=q.q5.b_1106
        )
